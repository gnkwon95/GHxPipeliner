...
def main():
    ...
    wc_df.to_csv('/tmp/wc.csv')
    author_df.to_csv('/tmp/author.csv')


    s3_client.upload_file('/tmp/wc.csv', bucket_name, f"premart/wordcount/{filename}.csv")
    s3_client.upload_file('/tmp/author.csv', bucket_name, f"premart/author/{filename}.csv")
